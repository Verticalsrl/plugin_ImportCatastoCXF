...
class ImportCatastoCXF:
    ...
    def select_input_file(self):
        ...
        #Recupero eventualmente la directory di salvataggio per decidere se abilitare o meno il pulsante IMPORT
        dirname_check = self.dlg.dirBrowse_txt.text()
        if (filename and dirname_check):
            self.dlg.importBtn.setEnabled(True);
        else:
    ...
    def select_output_dir(self):
        ...
        #Recupero eventualmente il nome file CXF da caricare per decidere se abilitare o meno il pulsante IMPORT
        filename_check = self.dlg.fileBrowse_txt.text()
        if (dirname and filename_check):
            self.dlg.importBtn.setEnabled(True);
        else:
            self.dlg.importBtn.setEnabled(False);
    ...
    def import_action(self):
        ...
        #dest_dir = "dbname=tucatuca host=localhost port=5432 user=postgres password=ARTURO active_schema=public"
        #write_foglio(foglio, dest_dir, point_borders=False, format_name='PostgreSQL')
        #ERROR 1: PostgreSQL driver doesn't currently support database creation.
        
        '''
        Aggiungo il layer su mappa: funzione NON IMPLEMENTATA in quanto non so il NOME DEL LAYER GENERATO a prescindere!!
        '''
        if (chkbox_value==True):
			...
			layer_linee_name = 'CATASTO_LINEE_%s' % (pedice)
			...
			layer_linee = QgsVectorLayer(layer_path+"\\"+layer_linee_name+".shp", layer_linee_name, "ogr")
            lista_layer_to_load=[layer_simboli, layer_fiduciali, layer_testi, layer_linee, layer_bordi]
			...
			layer_linee.loadNamedStyle(os.getenv("HOME")+'/.qgis2/python/plugins/ImportCatastoCXF/qml_base/catasto_linee.qml')
